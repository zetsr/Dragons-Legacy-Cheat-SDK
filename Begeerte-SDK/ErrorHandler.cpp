...
namespace ErrorHandlerNs {
    ...
    // 线程函数，用于显示错误窗口
    DWORD WINAPI ErrorMessageThread(LPVOID lpParam) {
        std::string* message = static_cast<std::string*>(lpParam);
        MessageBoxA(NULL, message->c_str(), "应用程序错误", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
    ...
    LONG WINAPI UnhandledExceptionFilter(EXCEPTION_POINTERS* ExceptionInfo) {
        ...
        // 创建独立线程显示错误窗口
        std::string* threadMessage = new std::string(displayText); // 动态分配，确保线程独立
        HANDLE hThread = CreateThread(NULL, 0, ErrorMessageThread, threadMessage, 0, NULL);
        if (hThread) {
            // 分离线程，不等待其完成
            CloseHandle(hThread);
        }
        else {
            // 如果线程创建失败，直接显示（但可能随进程退出）
            MessageBoxA(NULL, displayText.c_str(), "应用程序错误", MB_OK | MB_ICONERROR | MB_SYSTEMMODAL);
            delete threadMessage; // 清理内存
        }
